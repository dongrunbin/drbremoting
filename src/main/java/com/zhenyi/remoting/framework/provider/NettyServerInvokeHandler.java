...
public class NettyServerInvokeHandler extends SimpleChannelInboundHandler<byte[]>
{
...
//		}
		if (ctx.channel().isWritable())
		{
			RequestEntity request = SerializerEngine.deserialize(buffer, RequestEntity.class, PropertyConfigeHelper.getSerializeType().toString());
			...
			byte[] data = SerializerEngine.serialize(response, PropertyConfigeHelper.getSerializeType().toString());
